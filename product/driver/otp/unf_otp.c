...
HI_S32 HI_UNF_OTP_GetIDWordLockFlag(HI_BOOL *pbLockFlag)
{
    ...
    value = HI_UNF_OTP_Read(OTP_INTERNAL_DATALOCK_0); //aligned by 4 bytes
...
HI_S32 HI_UNF_OTP_LockIDWord(HI_VOID)
{
    ...
    /* ID_WORD_lock */
    u32Value = HI_UNF_OTP_Read(OTP_INTERNAL_DATALOCK_0); //aligned by 4 bytes
    ...
    if(0 == u8Value)
    {
        ...
        ret = HI_UNF_OTP_WriteByte(OTP_INTERNAL_DATALOCK_0 + 1, u8Value);
        if(HI_SUCCESS != ret)
        {
    ...
    u32Value = HI_UNF_OTP_Read(OTP_INTERNAL_PVLOCK_1); //aligned by 4 bytes
    ...
    if(0 == u8Value)
    {
        ...
        ret = HI_UNF_OTP_WriteByte(OTP_INTERNAL_PVLOCK_1, u8Value);
        if(HI_SUCCESS != ret)
        {
...
HI_S32 HI_UNF_OTP_BurnToSecureChipset(HI_VOID)
{
    ...
    u32Value = HI_UNF_OTP_Read(OTP_ADVCA_ID_WORD_ADDR);
    ...
    /* echo write 0x04 0x01 > /proc/msp/otp //secure_chip_flag */
    ret = HI_UNF_OTP_WriteByte(OTP_INTERNAL_PV_1, 0x01);
    if(HI_SUCCESS != ret)
    {
    ...
    /* echo write 0x19 0x08 > /proc/msp/otp  //right_ctrl_en */
    ret = HI_UNF_OTP_WriteByte(OTP_RIGHT_CTRL_EN_ADDR, 0x08);
    if(HI_SUCCESS != ret)
    {
    ...
    /*
        ...
        echo write 0xab 0x6e > /proc/msp/otp  //Advca ID_WORD
    */
    ret = HI_UNF_OTP_WriteByte(OTP_ADVCA_ID_WORD_ADDR, 0x53);
    ret |= HI_UNF_OTP_WriteByte(OTP_ADVCA_ID_WORD_ADDR + 1, 0xe9);
    ret |= HI_UNF_OTP_WriteByte(OTP_ADVCA_ID_WORD_ADDR + 2, 0xdb);
    ret |= HI_UNF_OTP_WriteByte(OTP_ADVCA_ID_WORD_ADDR + 3, 0x6e);
    if(HI_SUCCESS != ret)
    {
    ...
    ret = HI_UNF_OTP_WriteByte(OTP_SCS_EN_BAK_ADDR, 0x81);
    if(HI_SUCCESS != ret)
    {
    ...
    ret = HI_UNF_OTP_WriteByte(OTP_JTAG_MODE_BAK_ADDR, 0x42);
    if(HI_SUCCESS != ret)
    {
    ...
    ret = HI_UNF_OTP_WriteByte(OTP_RIGHT_CTRL_EN_BAK_ADDR, 0xff);
    if(HI_SUCCESS != ret)
    {
    ...
    ret = HI_UNF_OTP_WriteByte(OTP_SELF_BOOT_DIABLE_BAK_ADDR, 0x11);
    if(HI_SUCCESS != ret)
    {
