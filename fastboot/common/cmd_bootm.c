...
extern int HI_UNF_I2C_Read(unsigned int u32I2cNum, unsigned char u8DevAddress, unsigned int u32RegAddr,
				unsigned int u32RegAddrCount, unsigned char *pu8Buf, unsigned int u32Length);
extern int HI_UNF_I2C_Write(unsigned int u32I2cNum, unsigned char u8DevAddress, unsigned int u32RegAddr,
				unsigned int u32RegAddrCount, unsigned char *pu8Buf, unsigned int u32Length);

int read_eeprom_data(int regaddr, int length, unsigned char *buffer)
{
	int idx = 0;
	...
	for (idx = 0; idx < length; idx++) {
		realaddr = regaddr + idx;
		ret = HI_UNF_I2C_Read(2, 0xAD, realaddr, 1, &buffer[idx], 1);
		mdelay(5);
		if (ret != 0) {
			printf("EEPROM read register %#x failed with %#x.\n", realaddr, ret);
			return -1;
		}
	}
...
int write_eeprom_data(int regaddr, int length, unsigned char *buffer)
{
	int idx = 0;
	unsigned int realaddr = 0;
	int ret;
	...
	for (idx = 0; idx < length; idx++) {
		realaddr = regaddr + idx;
		ret = HI_UNF_I2C_Write(2, 0xAC, realaddr, 1, &buffer[idx], 1);
		mdelay(5);
		if (ret != 0) {
			printf("EEPROM write register %02x failed with %#x.\n", realaddr, ret);
			return -1;
		}
	}
...
static void select_kernel_bootm(struct partition_update_info *kernel_part)
{
	if (kernel_part->update_flag == BOOT_UPDATE) {
		kernel_part->update_flag = BOOT_NORMAL;
		if (kernel_part->which_partition == PARTITION_BACKUP) {
			printf("kernel boot from mtd11\n");
			setenv("bootcmd", "nand read 1000000 ca00000 800000;bootm 1000000");
		} else {
			printf("kernel boot from mtd8\n");
			setenv("bootcmd", "nand read 1000000 2200000 800000;bootm 1000000");
		}
	} else if (kernel_part->update_flag == BOOT_NORMAL) {
		if (kernel_part->partition_update_state == UPDATE_RUNNING) {
			if (kernel_part->which_partition == PARTITION_BACKUP) {
				printf("kernel boot from mtd8\n");
				kernel_part->which_partition = PARTITION_PRIMARY;
				setenv("bootcmd", "nand read 1000000 2200000 800000;bootm 1000000");
			} else {
				printf("kernel boot from mtd11\n");
				kernel_part->which_partition = PARTITION_BACKUP;
				setenv("bootcmd", "nand read 1000000 ca00000 800000;bootm 1000000");
			}
		} else {
			if (kernel_part->which_partition == PARTITION_BACKUP) {
		...
		kernel_part->update_flag = BOOT_NORMAL;
		kernel_part->which_partition = PARTITION_PRIMARY;
		kernel_part->partition_update_state = UPDATE_FINISH;
		setenv("bootcmd", "nand read 1000000 2200000 800000;bootm 1000000");
	}
}

static unsigned int get_rootfs_partition(struct partition_update_info *root_part)
{
	if (root_part->which_partition == PARTITION_BACKUP) {
		printf("rootfs boot from mtd12\n");
		return 12;
	} else {
		printf("rootfs boot from mtd9\n");
		return 9;
	}
}

static unsigned int get_swap_rootfs_partition(struct partition_update_info *root_part)
{
	if (root_part->which_partition == PARTITION_BACKUP) {
		printf("rootfs boot from mtd9\n");
		root_part->which_partition = PARTITION_PRIMARY;
		return 9;
	} else {
		printf("rootfs boot from mtd12\n");
		root_part->which_partition = PARTITION_BACKUP;
		return 12;
	}
}
...
static void select_root_app_args(struct partition_update_info *root_part,
				struct partition_update_info *app_part)
{
	...
	if (root_part->update_flag == BOOT_UPDATE) {
		...
		rootfs_mtd = get_rootfs_partition(root_part);
	} else if (root_part->update_flag == BOOT_NORMAL) {
		if (root_part->partition_update_state == UPDATE_RUNNING)
			rootfs_mtd = get_swap_rootfs_partition(root_part);
		else
			rootfs_mtd = get_rootfs_partition(root_part);
	} else {
		printf("boot rootfs from mtd9\n");
		root_part->update_flag = BOOT_NORMAL;
		root_part->which_partition = PARTITION_PRIMARY;
		root_part->partition_update_state = UPDATE_FINISH;
		rootfs_mtd = 9;
	}

	if (app_part->update_flag == BOOT_UPDATE) {
		app_part->update_flag = BOOT_NORMAL;
		appfs_mtd = get_appfs_partition(app_part);
	} else if (app_part->update_flag == BOOT_NORMAL) {
		if (app_part->partition_update_state == UPDATE_RUNNING)
			appfs_mtd = get_swap_appfs_partition(app_part);
		else
			appfs_mtd = get_appfs_partition(app_part);
	} else {
...
int select_boot_partition(void)
{
	unsigned int offset = offsetof(struct visking_product_info, how_boot);
	...
	printf("rootfs partition: flag %u, which %u, state %u.\n",
		boot_partition.rootfs.update_flag, boot_partition.rootfs.which_partition, boot_partition.rootfs.partition_update_state);
	printf("appfs partition: flag %u, which %u, state %u.\n",
		boot_partition.appfs.update_flag, boot_partition.appfs.which_partition, boot_partition.appfs.partition_update_state);
#endif

	select_kernel_bootm(&boot_partition.kernel);

	select_root_app_args(&boot_partition.rootfs, &boot_partition.appfs);

	return write_eeprom_data(offset, sizeof(struct boot_info), (unsigned char *)&boot_partition);
}
/*
U_BOOT_CMD(
	swap,	0,	0,	select_boot_partition,
	"swap boot partition",
	""
);
*/
