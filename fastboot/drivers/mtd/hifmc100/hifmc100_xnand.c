					...
					u8 oob_bitsflip)
{
	...
	if (oob_bitsflip <= xnand->ecc_strength){
        printf("check bitsflip %d and ecc strength %d\n",oob_bitsflip,xnand->ecc_strength);
		return true;
    }

	/* bbm != 0xFF && ecc fail && bbm > 2 bits fliping */
	if (hweight8(bbm) < 6){
        printf("check bad oob value %02X%02X%02X%02X%02X\n",
           xnand->oobbuf[0],xnand->oobbuf[1],xnand->oobbuf[2],xnand->oobbuf[3],xnand->oobbuf[4]);
		return true;
    }
...
static u32 hifmc100_ecc_strength_reg(u32 value, u32 _to_reg)
{
    ...
    printf("ec strength value %d reg %d\n",value,_to_reg);
