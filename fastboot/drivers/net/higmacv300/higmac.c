...
static void parse_module_parameters(void)
{
    ...
    higmac_board_info[GSF0_PORT1].higmac_ld.conn_switch = 1;
    higmac_board_info[GSF0_PORT1].higmac_ld.conn_speed = _1000BASET;
#endif
#if GMAC_AT_LEAST_3PORT
...
static int higmac_net_adjust_link(struct higmac_netdev_local *ld)
{
	...
	if (phy_addr == INVALID_PHY_ADDR && !ld->conn_switch)
		return stat;
...
link_failed:
	printf("ETH%d: PHY(%s, phyaddr=%d, %s) n  ot link!\n",
			ld->index,
			higmac_board_info[ld->index].mii_name,
...
link_on:
	printf("ETH%d: %sPHY(phyaddr=%d, %s) link UP: DUPLEX=%s : SPEED=%s\n",
			ld->index,ld->conn_switch ? "switch, ignore ":"",
			higmac_board_info[ld->index].phy_addr,
			phy_intf_str[higmac_board_info[ld->index].phy_intf],
